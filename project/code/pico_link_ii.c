...
//------------用户替换函数需要包含的头文件-----------------------
#include "zf_common_headfile.h" // 逐飞CH32V307库版本3.1.8
...
#define SPI_CS_LOW gpio_set_level(A15, 0)
// SPI CS引脚拉高, 需要自己找函数定义
#define SPI_CS_HIGH gpio_set_level(A15, 1)
// 全双工SPI发送多个字节, 需要自己找函数定义, array:发送数据首地址, length:传输长度
#define SPI_SEND_BYTES(array, length) spi_write_8bit_array((SPI_1), (array), (length))
// 全双工SPI读取多个字节, 需要自己找函数定义, buffer:接收数据首地址, length:传输长度
#define SPI_READ_BYTES(buffer, length) spi_read_8bit_array((SPI_1), (buffer), (length))
